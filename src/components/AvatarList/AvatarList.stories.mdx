import { Meta, Story, Props, Preview } from "@storybook/addon-docs/blocks";
import { select, text, boolean, number } from '@storybook/addon-knobs'
import AvatarSpec from '../../utilities/specs/avatarList.specs'
import AvatarList from './'
import { Avatar} from '../index'

<Meta
  title="Components/Structural/AvatarList"
  component={AvatarList}
  parameters={{
      design: {
        type: 'figma',
        url: 'https://www.figma.com/file/Wp9mDxTvWicSTWtezj2ImkMP/HSDS-Product?node-id=2582%3A10275'
      }
    }}
/>

# AvatarList

An AvatarList component displays an array of `Avatars`. This component has a max number of Avatars it will render. Any additional Avatars will be consolidated into a `+` counter that appears at the end.


<Preview>
  <Story name="Default">
    <div style={{ background: '#eee', padding: 10 }}>
      <AvatarList max={number('max', 14)} grid={boolean('grid', false)} center={boolean('centered', false)} stack={boolean('stack', false)} size={select('size', ['xs', 'sm', 'md', 'lg'])}>
        {AvatarSpec.generate(20).map(avatar => {
          const { name, image, status } = avatar
          return (
            <Avatar
              image={image}
              key={name}
              name={name}
              status={status}
            />
          )
        })}
      </AvatarList>
    </div>
  </Story>
</Preview>

#### Reference

- **Designer**: Buzz
- **Engineering**: Juan Pablo / PL
- **Usage**: Everywhere

<br />
<br />

### Props

<Props of={AvatarList} />
