...
The `onSelect` callback passes a few useful parameters specially when multiple selection is enabled:

```js
/**
 *
 * @param {string | object | array} selection The current selection, if multiple selection enabledm, then it is an array
 * @param {function} clearSelection Function that clears the current selection, useful in multiple selection cases to enable a "clear all" option
 * @param {string | object} selectedItem The item that was clicked, useful in multiple selection cases
 */
function onSelect(selection, clearSelection, clickedItem) {}
```
