...
- `renderCell`: To customize how each cell renders it corresponding data on this column. A function that takes the corresponding data and returns a React Component/Element, its arguments are the data provided by `columnKey` above, if using nested data, access it by replaceing the dot (`.`) with a slash (`_`), for example: `customer.email` in `columnKey` becomes `customer_email` in `renderCell` (see code example below).
...
const columns = [
  ...
  },
  {
    ...
    width: '50%',
    align: 'left',
    renderCell: ({ customer_name, customer_email }) => ( // replace . with _
        <div>
