...
- `selectKey`: A unique key from your data to identify selection, defaults to `id`
- `onSelectRow`: Callback that gives you an array of selected items (by the key provided in `selectKey` which by default is `id`)
