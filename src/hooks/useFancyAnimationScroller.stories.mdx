...
- `classNames`: Optional classnames to apply on certain circumnstances.
  - `targetReached`: A classname to add or remove as the "target" above (ie the header) is reached (scrollable content all the way to the top). Useful if you want to animate the contents of the header somehow.
- `targets`:
