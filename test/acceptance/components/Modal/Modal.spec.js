...
describe('Open/Close', () => {
  ...
  it('should be able to open with prop change', done => {
    const wrapper = mount(<Modal cardClassName="modal" />)
  ...
  it('should be able to close with prop change', done => {
    const wrapper = mount(<Modal cardClassName="modal" isOpen timeout={0} />)
...
describe('Scrollable', () => {
  it('should not extend beyond the Modal (height)', done => {
    ...
    wait(100).then(() => {
      expect($('.c-Modal__innerWrapper').height()).toBeGreaterThan(
        $('.c-ModalBody__scrollable').height()
      )
      done()
    })
  })
